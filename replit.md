...
- July 19, 2025. **CRITICAL BUG FIXED: Technician Service Access Restored** - Resolved critical production issue where technicians could not access their assigned services through `/api/my-services` endpoint. Problem was caused by improper session cookie settings: `secure: true` and `sameSite: "none"` prevented cookies from working on localhost and Replit platform. Fixed by implementing environment-aware cookie settings: `secure: false` and `sameSite: "lax"` for development, `secure: true` and `sameSite: "none"` for production. Also reset all technician passwords to consistent hash. All four technicians (Petar: 10 services, Gruica: 7 services, Jovan: 1 service, Nikola: 6 services) can now successfully login and access their assigned services. This resolves the urgent issue preventing technicians from working.
