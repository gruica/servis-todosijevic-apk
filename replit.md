...
- July 15, 2025. **Comprehensive Security Framework Implemented** - Complete security hardening for web deployment implemented with multiple layers of protection. Added helmet middleware for security headers, rate limiting (100 requests per 15 minutes, 5 login attempts per 15 minutes), input sanitization, SQL injection protection, CSRF protection, route parameter validation, role-based access control, and security logging. All endpoints now require authentication and proper authorization. Admin-only endpoints protected with requireRole(['admin']) middleware. SQL admin panel restricted to SELECT queries only. Excel export/import endpoints secured. Security events logged with IP tracking and user identification. Application now fully protected against unauthorized access and common web vulnerabilities.
- July 15, 2025. **Device Categories Expanded** - Added 6 new device categories to system: Ugradna sudo mašina (Built-in dishwasher), Aspirator (Exhaust fan), Indukciona ploča (Induction cooktop), Plinska ploča (Gas cooktop), Mikrotalasna rerna (Microwave oven), Kombinovani frižider (Combined refrigerator). Categories available in all user interfaces with proper icons and database integration. Total system now supports 16 device categories.
