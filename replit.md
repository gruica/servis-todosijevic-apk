...
- July 18, 2025. **GSM Modem-Only SMS System with WiFi Support Completed** - Completely removed all other SMS services (Twilio, etc.) and implemented dedicated GSM modem-only system using physical SIM card 067028666. Created dedicated GSM modem admin page (/admin/gsm-modem) with simplified interface for direct SMS communication through either USB connection or WiFi network connectivity. Implemented GSM-only SMS service (gsm-only-sms-service.ts) removing all fallback providers. Updated API endpoints to use only GSM modem functionality with dual connection support (USB/WiFi). Added automatic COM port detection, TCP/IP WiFi communication, configuration options for baud rate, SIM card number, WiFi host/port settings, test SMS functionality, status monitoring, and restart capabilities. System now supports both USB and WiFi connection types for maximum installation flexibility. Ready for physical hardware testing with either connection method.
