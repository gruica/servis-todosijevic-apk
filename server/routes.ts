...
export function setupSecurityEndpoints(app: Express, storage: IStorage) {
  ...
  // ===== ADMIN ENDPOINT ZA SERVISNI IZVJEŠTAJ =====
  // GET /api/admin/services/:id/report - Generiše kompletan izvještaj sa servisa
  app.get("/api/admin/services/:id/report", jwtAuth, requireRole(['admin']), async (req, res) => {
    try {
      const serviceId = parseInt(req.params.id);
      ...
      console.log(`[ADMIN SERVICE REPORT] Generiram izvještaj za servis ${serviceId}`);
      
      // Dohvati kompletan servis sa svim relacijama
      const service = await storage.getServiceById(serviceId);
      if (!service) {
        return res.status(404).json({ error: "Servis nije pronađen" });
      }
      
      // Dohvati uklonjene delove
      const removedParts = await storage.getRemovedPartsByService(serviceId);
      
      // Dohvati troškove servisa (ako postoje)
      let costs: any[] = [];
      let totals = { subtotal: 0, tax: 0, total: 0 };
      
      try {
        const completionReport = await storage.getServiceCompletionReport(serviceId);
        if (completionReport && completionReport.costLines) {
          costs = completionReport.costLines.map((cost: any) => ({
            description: cost.description || 'Usluga serviranja',
            isWarranty: service.isWarranty || false,
            quantity: parseFloat(cost.quantity) || 1,
            unitPrice: parseFloat(cost.unitPrice) || 0,
            discount: parseFloat(cost.discount) || 0,
            total: parseFloat(cost.total) || 0
          }));
          
          // Izračunaj totale
          totals.subtotal = costs.reduce((sum, cost) => sum + cost.total, 0);
          totals.tax = totals.subtotal * 0.21; // 21% PDV
          totals.total = totals.subtotal + totals.tax;
        }
      } catch (error) {
        console.log(`[ADMIN SERVICE REPORT] Nema troškova za servis ${serviceId}`);
      }
      
      // Kompanija informacije - hardkodovane za Frigo Sistem Todosijević
      const companyInfo = {
        name: "TEHNO PLUS",
        address: "Josipa Broza Tita br 47, Podgorica",
        phone: "020/345454",
        email: "info@tehnoplus.me",
        pib: "02305454",
        pdv: "30/31-015017",
        website: "www.tehnoplus.me"
      };
      
      // Kreiraj ServiceReportDTO
      const reportDTO = {
        companyInfo,
        service: {
          id: service.id,
          orderNumber: service.id.toString(), // koristimo ID kao broj naloga
          createdAt: service.createdAt,
          status: service.status,
          priority: service.priority,
          description: service.description,
          isWarranty: service.isWarranty,
          warrantyType: service.warrantyType,
        ...
        client: {
          ...
          pib: service.client.pib,
          pdv: service.client.pdv
        },
        appliance: {
          category: service.appliance.category.name,
          manufacturer: service.appliance.manufacturer.name,
          model: service.appliance.model,
          serialNumber: service.appliance.serialNumber,
          purchaseDate: service.appliance.purchaseDate
        },
        ...
        dates: {
          orderDate: service.createdAt,
          scheduledDate: service.scheduledDate,
          startDate: service.startDate,
          completionDate: service.completionDate,
          customerNotifiedAt: service.customerNotifiedAt
        },
        techReport: {
          initialDiagnosis: service.initialDiagnosis,
          workPerformed: service.workPerformed,
          machineNotes: service.machineNotes,
          finalNotes: service.finalNotes,
          clientUnavailableReason: service.clientUnavailableReason,
          reschedulingNotes: service.reschedulingNotes,
          devicePickupNotes: service.devicePickupNotes,
          repairFailureDetails: service.repairFailureDetails,
          warrantyClaimDetails: service.warrantyClaimDetails
        },
        removedParts: removedParts.map(part => ({
          partName: part.partName,
          ...
          currentLocation: part.currentLocation,
          partStatus: part.partStatus,
          isReinstalled: part.isReinstalled,
          repairCost: part.repairCost
        })),
        costs,
        totals
      ...
      console.log(`[ADMIN SERVICE REPORT] Uspešno kreiran izvještaj za servis ${serviceId}`);
      res.json(reportDTO);
      
    } catch (error) {
      console.error("Greška pri kreiranju servisnog izvještaja:", error);
      res.status(500).json({ 
        error: "Greška pri kreiranju servisnog izvještaja", 
        message: error instanceof Error ? error.message : String(error) 
      });
    }
  });
