...
Encryption: https://keys.openpgp.org/search?q=info@frigosistemtodosijevic.me`);
  ...
  app.get("/api/admin/billing/beko/enhanced", jwtAuth, async (req, res) => {
    try {
      ...
      // Definišemo SVE Beko brendove koji se fakturišu zajedno
      ...
      // ENHANCED LOGIKA: Hvata sve završene servise za Beko brendove
...
export function setupSecurityEndpoints(app: Express, storage: IStorage) {
  ...
  // ENDPOINT ZA ŠTAMPANJE PRIJAVA POSLOVNIH PARTNERA
  // Dodato na zahtev korisnika - omogućava business partnerima da štampaju svoje prijave
  app.get("/api/business/services/:id/print", jwtAuth, async (req, res) => {
    try {
      const serviceId = parseInt(req.params.id);
      const userId = req.user?.id;
      
      console.log(`[PRINT SERVICE] Business partner ${userId} traži štampanje servisa ${serviceId}`);
      
      // Provera da je korisnik business partner ili admin
      if (!req.user || !['business_partner', 'admin'].includes(req.user.role)) {
        return res.status(403).json({ 
          error: "Nemate dozvolu za pristup ovoj funkcionalnosti" 
        });
      }
      
      // Dobijanje servisa i povezanih podataka
      const service = await storage.getService(serviceId);
      if (!service) {
        return res.status(404).json({ 
      ...
      // Provera da business partner može pristupiti ovom servisu
      if (req.user.role === 'business_partner' && service.businessPartnerId !== userId) {
        console.log(`[PRINT SERVICE] BEZBEDNOST: Partner ${userId} pokušava pristup servisu ${serviceId} koji mu ne pripada`);
        return res.status(403).json({ 
          error: "Nemate dozvolu za štampanje ovog servisa" 
        });
      }
      
      // Dobijanje dodatnih podataka
      const client = await storage.getClient(service.clientId);
      const appliance = await storage.getAppliance(service.applianceId);
      const category = appliance ? await storage.getApplianceCategory(appliance.categoryId) : null;
      const manufacturer = appliance ? await storage.getManufacturer(appliance.manufacturerId) : null;
      ...
      // Formatiranje datuma za štampanje
      const formatDateForPrint = (dateStr: string | null) => {
        if (!dateStr) return 'Nije uneto';
      ...
      // Prevod statusa za štampanje
      const translateStatusForPrint = (status: string) => {
        const statusMap: Record<string, string> = {
          ...
          customer_refused_repair: "Klijent odbio servis",
      ...
      // Kreiranje HTML sadržaja za štampanje
      const printContent = `
        <!DOCTYPE html>
        <html lang="sr">
        <head>
          ...
          <title>Prijava kvara #${service.id}</title>
          <style>
        ...
        <body>
          <div class="header">
            <div class="company-name">Frigo Sistem Todosijević</div>
            <div>Profesionalni servis bele tehnike</div>
            <div class="document-title">PRIJAVA KVARA #${service.id}</div>
          </div>
          ...
          <div class="info-grid">
            <div class="info-section">
              ...
              <div class="field">
                ...
                <span class="field-value">${client?.fullName || 'Nije uneto'}</span>
              </div>
              <div class="field">
                <span class="field-label">Telefon:</span>
                <span class="field-value">${client?.phone || 'Nije uneto'}</span>
              </div>
              <div class="field">
                <span class="field-label">Email:</span>
                <span class="field-value">${client?.email || 'Nije uneto'}</span>
              </div>
              <div class="field">
                <span class="field-label">Adresa:</span>
                <span class="field-value">${client?.address || 'Nije uneto'}</span>
              </div>
              <div class="field">
                <span class="field-label">Grad:</span>
                <span class="field-value">${client?.city || 'Nije uneto'}</span>
              </div>
            </div>
            ...
            <div class="info-section">
              ...
              <div class="field">
                ...
                <span class="field-value">${category?.name || 'Nije uneto'}</span>
              </div>
              <div class="field">
                <span class="field-label">Proizvođač:</span>
                <span class="field-value">${manufacturer?.name || 'Nije uneto'}</span>
              </div>
              <div class="field">
                <span class="field-label">Model:</span>
                <span class="field-value">${appliance?.model || 'Nije uneto'}</span>
              </div>
              <div class="field">
                <span class="field-label">Serijski broj:</span>
                <span class="field-value">${appliance?.serialNumber || 'Nije uneto'}</span>
              </div>
              <div class="field">
                <span class="field-label">Status garancije:</span>
                <span class="field-value">${service.warrantyStatus || 'Nije uneto'}</span>
              </div>
            </div>
          </div>
          ...
          <div class="info-grid">
            <div class="info-section">
              <div class="section-title">PODACI O SERVISU</div>
              <div class="field">
                <span class="field-label">Status:</span>
                <span class="field-value status-badge">${translateStatusForPrint(service.status)}</span>
              </div>
              <div class="field">
                <span class="field-label">Datum kreiranja:</span>
                <span class="field-value">${formatDateForPrint(service.createdAt)}</span>
              </div>
              <div class="field">
                <span class="field-label">Zakazano za:</span>
                <span class="field-value">${formatDateForPrint(service.scheduledDate)}</span>
              </div>
              <div class="field">
                <span class="field-label">Završeno:</span>
                <span class="field-value">${formatDateForPrint(service.completedDate)}</span>
              </div>
              <div class="field">
                <span class="field-label">Cena:</span>
                <span class="field-value">${service.cost ? service.cost + ' €' : 'Nije uneto'}</span>
              </div>
            </div>
            
            <div class="info-section">
              <div class="section-title">SERVISER I PARTNER</div>
              <div class="field">
                <span class="field-label">Serviser:</span>
                <span class="field-value">${technician?.fullName || 'Nije dodeljen'}</span>
              </div>
              ${technician?.specialization ? `
              <div class="field">
                <span class="field-label">Specializacija:</span>
                <span class="field-value">${technician.specialization}</span>
              </div>
              ` : ''}
              <div class="field">
                <span class="field-label">Poslovni partner:</span>
                <span class="field-value">${service.partnerCompanyName || req.user.companyName || 'Nije uneto'}</span>
              </div>
              <div class="field">
                <span class="field-label">Potpuno popravljen:</span>
                <span class="field-value">${service.isCompletelyFixed === true ? 'Da' : service.isCompletelyFixed === false ? 'Ne' : 'Nije uneto'}</span>
              </div>
            </div>
          </div>
          
          <div class="description-section">
            <div class="section-title">OPIS PROBLEMA</div>
            <div style="margin-top: 10px; white-space: pre-wrap;">${service.description || 'Nema opisa'}</div>
          </div>
          
          ${service.technicianNotes ? `
          <div class="description-section">
            <div class="section-title">NAPOMENE SERVISERA</div>
            <div style="margin-top: 10px; white-space: pre-wrap;">${service.technicianNotes}</div>
          </div>
          ` : ''}
          
          <div class="footer">
            <div>Frigo Sistem Todosijević - Profesionalni servis bele tehnike</div>
            <div>Dokument kreiran: ${new Date().toLocaleDateString('sr-RS')} u ${new Date().toLocaleTimeString('sr-RS')}</div>
            <div>Štampano od strane: ${req.user.companyName || req.user.fullName}</div>
          </div>
        </body>
        </html>
      `;
      
      console.log(`✅ [PRINT SERVICE] Generisan HTML za štampanje servisa ${serviceId}`);
      
      // Postavke odgovora za preuzimanje HTML-a (može se štampati direkt iz browsera)
      res.setHeader('Content-Type', 'text/html; charset=utf-8');
      res.setHeader('Content-Disposition', `inline; filename="Prijava_kvara_${serviceId}.html"`);
      res.send(printContent);
      
    } catch (error) {
      console.error("[PRINT SERVICE] Greška pri generisanju štampanja:", error);
      res.status(500).json({ 
        error: "Greška pri generisanju dokumenta za štampanje",
        message: error instanceof Error ? error.message : 'Nepoznata greška'
      });
    }
  });
