...
export function setupSecurityEndpoints(app: Express, storage: IStorage) {
  ...
  // ============================================================================
  // 🔧 JEDNOSTAVAN ADMIN SYNC ENDPOINT - popravka za retroaktivnu sinhronizaciju
  // ============================================================================
  
  // POST /api/admin/sync-missing-simple - Jednostavna retroaktivna sinhronizacija supplier orders
  app.post("/api/admin/sync-missing-simple", jwtAuth, async (req, res) => {
    try {
      if (!req.user || req.user.role !== 'admin') {
        return res.status(403).json({ error: "Samo admin može pokrenuti jednostavnu sinhronizaciju" });
      }
      
      console.log(`🔧 [SIMPLE-ADMIN SYNC] Korisnik ${req.user.fullName} pokreće jednostavnu retroaktivnu sinhronizaciju...`);
      
      const result = await storage.syncMissingSupplierOrdersSimple();
      
      res.json({
        success: true,
        message: `Jednostavna retroaktivna sinhronizacija završena`,
        created: result.created,
        errors: result.errors,
        details: result.errors.length > 0 ? 
          `Kreirano ${result.created} supplier orders, ${result.errors.length} grešaka` :
      ...
      console.error('[SIMPLE-ADMIN SYNC] Greška pri jednostavnoj retroaktivnoj sinhronizaciji:', error);
      res.status(500).json({ 
        error: 'Greška pri jednostavnoj retroaktivnoj sinhronizaciji supplier orders',
        message: error instanceof Error ? error.message : 'Nepoznata greška'
      });
    }
  });
  ...
  // ============================================================================
  // 🔧 DIREKTAN SQL ADMIN SYNC ENDPOINT - zaobilaženje Drizzle schema problema
  // ============================================================================
  
  // POST /api/admin/sync-missing-sql - Direktna SQL retroaktivna sinhronizacija supplier orders
  app.post("/api/admin/sync-missing-sql", jwtAuth, async (req, res) => {
    try {
      if (!req.user || req.user.role !== 'admin') {
        return res.status(403).json({ error: "Samo admin može pokrenuti direktnu SQL sinhronizaciju" });
      }
      
      console.log(`🔧 [SQL-ADMIN SYNC] Korisnik ${req.user.fullName} pokreće direktnu SQL retroaktivnu sinhronizaciju...`);
      
      const result = await storage.syncMissingSupplierOrdersSQL();
      
      res.json({
        success: true,
        message: `Direktna SQL retroaktivna sinhronizacija završena`,
        created: result.created,
        errors: result.errors,
        details: result.errors.length > 0 ? 
          `Kreirano ${result.created} supplier orders, ${result.errors.length} grešaka` :
      ...
      console.error('[SQL-ADMIN SYNC] Greška pri direktnoj SQL retroaktivnoj sinhronizaciji:', error);
      res.status(500).json({ 
        error: 'Greška pri direktnoj SQL retroaktivnoj sinhronizaciji supplier orders',
        message: error instanceof Error ? error.message : 'Nepoznata greška'
      });
    }
  });
