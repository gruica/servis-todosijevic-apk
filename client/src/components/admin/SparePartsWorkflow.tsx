    ...
    <Card className="mb-4">
      <CardContent className="p-4">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm mb-3">
          <div>
            <span className="font-medium">Količina:</span> {order.quantity}
          </div>
          <div>
            <span className="font-medium">Hitnost:</span> 
            <Badge variant={order.urgency === 'high' ? 'destructive' : order.urgency === 'medium' ? 'secondary' : 'outline'} className="ml-1">
        ...
        {(order.supplierName || order.estimatedCost || order.actualCost) && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm mb-3 p-2 bg-muted rounded">
            {order.supplierName && (
              <div>
                <span className="font-medium">Dobavljač:</span> {order.supplierName}
              </div>
            )}
            {order.estimatedCost && (
              <div>
                <span className="font-medium">Procenjena cena:</span> {order.estimatedCost}
              </div>
            )}
            {order.actualCost && (
              <div>
                <span className="font-medium">Stvarna cena:</span> {order.actualCost}
              </div>
            )}
          </div>
        )}
